...
static Status_t ReadRegister(uint8_t reg, uint8_t* buffer)
{
    ...
    if (ESP_OK == i2c_master_transmit_receive(dev_handle, &reg, 1, buffer, 1, 1000))
    {
        return STATUS_OK;
    }
    else
    {
        ESP_LOGI("BME280", "ReadRegister failed");
...
static Status_t ReadMulti(uint8_t addr, uint8_t* dst, uint32_t size)
{
    if (ESP_OK == i2c_master_transmit_receive(dev_handle, &addr,1, dst, size, 1000))
    {
        return STATUS_OK;
    }
    else
    {
        ESP_LOGI("BME280", "ReadMulti failed");
        return STATUS_I2C_ERROR;
    }
}
...
static Status_t WriteRegister(uint8_t reg, uint8_t data)
{
	...
	buff[0] = reg;
	buff[1] = data;
    if (ESP_OK == i2c_master_transmit(dev_handle, buff, 2, 1000))
    {
        return STATUS_OK;
    }
    else
    {
        ESP_LOGI("BME280", "WriteRegister failed");
        return STATUS_I2C_ERROR;
    }
}
...
static Status_t GetTemperatureAndPressureCalibrationData()
{
    ...
    if (STATUS_OK != status)
    {
        ESP_LOGI("BME280", "Temperature and pressure calibration data reading failed");
        return status;
    }
...
static Status_t GetHumidityCalibrationData()
{
    ...
    if (STATUS_OK != status)
    {
        ESP_LOGI("BME280", "Humidity calibration data reading failed");
        return status;
    }
...
Status_t BME280_Init()
{
	...
	if (ESP_OK != i2c_master_transmit_receive(dev_handle, &reg, 1, &chip_id, 1, 1000))
	{
	    ESP_LOGE("BME280", "Chip ID read failed");
	    return STATUS_I2C_ERROR;
	}

	if (chip_id != 0x60)
	{
	    ESP_LOGE("BME280", "Wrong CHIP ID: 0x%02X", chip_id);
	    return STATUS_ERROR;
	}
	ESP_LOGI("BME280", "BME280 detected OK");
...
BME280_Data_t BME280_ReadData()
{
	...
	uint8_t data_reg = BME280_DATA_REG;
	if (ESP_OK != i2c_master_transmit_receive(dev_handle, &data_reg, 1, regData, 8, 1000))
    {
        ESP_LOGI("BME280", "Error reading sensor data");
